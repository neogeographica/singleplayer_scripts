...
These are the scripts (and associated config file and sounds) used to do the heavy lifting for the features described in the top-level readme.
...
The "sounds" directory contains a selection of error beeps. If you want to use these sounds, copy them to a location of your choice, where they will not get deleted.
...
You can define shortcuts_dir in the config file if you want the launcher to automatically create a shortcut file in your specified directory whenever it creates a new gamedir. You can also create shortcut files yourself by whatever means you want.
...
* If a shortcut file contains the path to a gamedir, it will launch that gamedir. Shortcuts created by the launcher are like this.
* If a shortcut file is empty (doesn't contain any non-whitespace), then it will just launch the gamedir that has the same basename as the shortcut.
* If you use the cleanup script on a shortcut, it will also delete the gamedir that the shortcut points to.
* If you use the cleanup script on a gamedir, it will also delete any shortcuts in the current shortcuts_dir that point to that gamedir.

Note about "empty" shortcuts: you will want to make sure that they do contain some whitespace, even just a single space or carriage return character. This is because some Linux configurations will not correctly recognize the filetype of zero-length files, which undermines the desired double-click behavior. So if you wanted to manually create an "empty"-style shortcut for the "hwjam2" gamedir, you could do:
```
echo > hwjam2.quake
```
to make sure to create a shortcut file that has a carriage return in it.
...
* If there are pak files in the gamedir, and expak is not installed, we can't figure out what maps there might be. Just activate the gamedir and don't load any map.
* Otherwise build a list of "loose" bsp files in the gamedir and in any of its paks.
...
If you have a gamedir that requires one of these basegames but doesn't mention that fact in any docs, you can create a .txt file in the gamedir to trigger quakelaunch to use the correct basegame. The file should contain "-quoth", "-hipnotic", or "-rogue" accordingly.
...
Most Quake engines don't support that though. I've modified my own build of Quakespasm to do it, and FTE can do it as shown in some examples in the conf file. If you have a Quake engine that can do it, you can set the relevant entries in the conf file.
...
* If you're set up to handle AD/Copper as a "basegame", the content will be installed in its own gamedir and launched accordingly.
* Otherwise, the script will exit without doing the installation. You'll need to do the install manually, by file merging or whatever works for you.
...
* Otherwise the gamedir will be launched normally without specifying any basegame. The script assumes you know what you're doing.

# Testing

If you want to test whether you've installed the expak module correctly, cd into your id1 directory and execute this:
```
...
You can also test the scripts from the shell command line. The scripts are meant to be used through desktop integration, but you can also run them manually.
...
If you installed the prereqs as above, and you ever need to uninstall them, you can do so with
```bash
