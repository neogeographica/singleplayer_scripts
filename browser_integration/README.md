...
The version in the "chrome_extension_manifest_v3" folder is an in-development version to handle extension changes in future versions of the Chrome browser. Currently it only works in Chrome 110.0.5469.0 or later (which, as of the last time this doc was updated, is only available on the beta or dev channels of Chrome builds). Google's roadmap will require using this version in June 2023, so at some point before then I'll switch to having this one be the stable default version. For now though it's probably only for the really curious web developer to look at.
